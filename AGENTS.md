...
- **`app/scraper/`** — Pure HTTP/HTML layer: `httpclient.py` (curl-cffi) and `parser.py` (BeautifulSoup). No business logic here. The curl-cffi browser fingerprint is the `IMPERSONATE` constant (env `SCRAPE_IMPERSONATE`, default **`chrome131`**) — the library default `"chrome"` triggers Cloudflare 403s and Amazon price-stripped pages; `chrome131` passes both from a residential IP. **Optional proxy/unblocker for price-watch fetches only:** from a **datacenter IP (Railway) the fingerprint alone is not enough** (Amazon/Cloudflare penalise datacenter IPs regardless of fingerprint), so `fetch_page_with_status(url, via_proxy=True)` routes through a proxy when configured. Config (pydantic, so `.env` **and** real env vars work): set **`SCRAPINGANT_API_KEY`** — the ScrapingAnt residential proxy URL is built from it automatically (`_resolve_proxy(render)` builds one URL per rendering tier), with `SCRAPINGANT_RENDER` (default **true**; enables the slow, credit-heavy JS-rendering tier as a last resort for JS-only pages — set **false** to disable that tier entirely) and `SCRAPINGANT_COUNTRY` (default **`DE`** → amazon.de returns EUR, not USD from a US IP). Alternatively `SCRAPE_PROXY_URL` (+ `SCRAPE_PROXY_VERIFY=false` for MITM-cert proxies) for any proxy/unblocker (this path has no rendering toggle, so it acts as a single proxy tier). Price-watch fetches use **`fetch_with_proxy_fallback(url, is_success)`** — a **three-tier** fetch: **direct first** (fast, free; short `PRICEWATCH_DIRECT_TIMEOUT`≈8s so it never hangs ~20s before falling back) → **proxy without rendering** (`browser=false`; fast + cheap, handles most protected shops incl. Amazon, whose price sits in the raw HTML) → **proxy with rendering** (`browser=true`; slow + credit-heavy, only for JS-only pages, gated by `SCRAPINGANT_RENDER` / `_RENDER_TIER_ENABLED`). `is_success(status, html)` judges each tier; the first usable result wins, so unprotected sites stay instant and free and rendering is a rare last resort. Price-watch fetches also send an **`Accept-Language`** header (`scrape_accept_language`, default `de-DE,…`) — without it Amazon serves the English page variant with different prices/formatting (e.g. `€44.40` / "RRP:" instead of `44,40 €` / "UVP:"), so the wrong price gets extracted. The high-volume Kleinanzeigen scraper (`fetch_page`/`fetch_pages`) and AdSearch URL validation never touch the proxy, so they never burn credits.
- **`app/services/`** — Business logic: `ScraperService` orchestrates scraping pipeline, `AIService` handles AI analysis (OpenAI-compatible API) with comparison prices, `SettingsService` reads runtime settings from DB. **`PriceExtractor`** (`price_extractor.py`) extracts price candidates from arbitrary HTML (JSON-LD → meta → visible text), ranks them by a **prominence score** (structured data + buy-box containers score high, strikethrough/UVP — via container class *or* text like "UVP:"/"statt" — and monthly-rate prices score low; ties broken by **frequency**, since the real buy-box price is echoed many times across the page while side prices appear once) so the probable **main price sorts first / becomes the `recommended` default without needing AI**, and re-finds the chosen price via a stored locator; **`PriceWatchService`** runs the price-check pipeline; **`NotificationService`** manages the generic in-app notification store.
